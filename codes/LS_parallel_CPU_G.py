...
def process_source(source_id):
    ...
    lsp_path = save_ls_to + 'lsp/' + f'{source_id}_{passbands}.npy'
    
    if os.path.exists(file_path) and not os.path.exists(lsp_path):
        ...
        if df.shape[0] >= 24:
            ...
            lsp = LombScargle(Time, mag, mag_err, nterms=1).power(
                frequencies, method="cython", normalization="psd"
            )
            
            # Save results
            np.save(save_ls_to + 'lsp/' + f'{source_id}_{passbands}.npy', np.vstack([frequencies,lsp]))
...
for source_id in tqdm.tqdm(ids):
    path='/idia/users/princy/project_obj_btw_MS_WD/gaia_periodograms/lsp/%d_%s.npy'%(source_id,passbands)
